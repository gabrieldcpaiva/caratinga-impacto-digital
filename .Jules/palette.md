...
## 2024-07-09 - Respecting Reduced Motion Preferences in JavaScript
**Learning:** Many interactive elements that use `Element.scrollIntoView()` default to `behavior: 'smooth'` without checking the user's OS-level accessibility preferences for reduced motion, which can trigger vestibular issues.
**Action:** Always conditionally apply smooth scrolling behavior in JavaScript by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` before invoking programmatic scroll animations.
