...
## 2024-06-21 - Keyboard Navigation Accessibility Focus Styles
**Learning:** Custom UI components lacked explicit `*:focus-visible` styles, and inputs used `:focus` which can disrupt mouse/touch users. Global focus visible styles are necessary for keyboard navigation accessibility.
**Action:** Always ensure explicit `*:focus-visible` (not `:focus`) styles are defined in root CSS files using appropriate contrast colors (e.g., `--accent-clay` for light themes and `white` for dark themes).
