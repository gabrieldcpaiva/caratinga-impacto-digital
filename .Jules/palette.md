...
## 2024-07-21 - Vestibular Accessibility & Smooth Scrolling
**Learning:** Hardcoding `behavior: 'smooth'` in JavaScript scroll methods (like `scrollIntoView`) ignores user system preferences for reduced motion, which can cause discomfort or nausea for users with vestibular disorders.
**Action:** Always conditionally apply smooth scrolling by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches`.
