...
## 2024-06-30 - Respecting vestibular preferences in JavaScript
**Learning:** Using `scrollIntoView({ behavior: 'smooth' })` unconditionally in JavaScript can cause issues for users with vestibular disorders by overriding browser-level reduced motion preferences.
**Action:** Always conditionally apply `behavior: 'smooth'` by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` to respect user settings.
