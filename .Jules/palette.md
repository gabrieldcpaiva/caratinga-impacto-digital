...
## 2024-05-24 - Keyboard Navigation Focus Indicators
**Learning:** Keyboard users struggled to navigate because interactive elements lacked explicit focus indicators, and relying solely on `:focus` disrupted mouse/touch users.
**Action:** Always ensure custom UI components have explicit `*:focus-visible` styles defined in root CSS files using appropriate contrast colors (e.g., `--accent-clay` for light themes, `white` for dark themes).
