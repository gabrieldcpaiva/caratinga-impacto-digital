...
## 2024-06-12 - Missing Focus Visible Styles
**Learning:** The project's root CSS files (`void-styles.css`, `proposta-styles.css`, and `gold-mold/css/styles.css`) lacked explicit `*:focus-visible` styles, which degrades keyboard accessibility as users cannot clearly see which element has focus.
**Action:** Always ensure that `*:focus-visible` styles are explicitly defined to support keyboard navigation without disrupting the experience for mouse/touch users. Use an appropriate high-contrast outline color (e.g., `white` for dark themes or a primary/accent color like `--accent-clay` for light themes).
