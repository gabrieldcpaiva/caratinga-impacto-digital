...
## 2024-06-23 - Focus Visible for Accessibility
**Learning:** Custom UI components without explicit focus styles fail keyboard accessibility. Relying solely on default browser outlines or `*:focus` can disrupt mouse users.
**Action:** Always add `*:focus-visible` styles with sufficient contrast (e.g., `--accent-clay` or `white`) to global stylesheets to ensure keyboard navigation is visible without breaking mouse experiences.
