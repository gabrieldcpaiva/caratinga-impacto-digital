...
## 2024-06-18 - Missing Keyboard Focus Indicators
**Learning:** Across the repository, custom UI components and elements (such as buttons and inputs) lacked explicit `*:focus-visible` styles in the root CSS files. This breaks keyboard navigation accessibility as users tabbing through the interface receive no clear visual indicator of which element currently has focus.
**Action:** Always ensure custom UI components have explicit `*:focus-visible` styles defined in the root CSS files (`gold-mold/css/styles.css`, `void-styles.css`, and `proposta-styles.css`), using appropriate contrast colors (e.g., `--accent-clay` for light themes and `white` for dark themes).
