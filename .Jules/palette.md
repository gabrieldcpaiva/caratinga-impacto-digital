...
## 2024-05-17 - Keyboard Navigation Focus States
**Learning:** During accessibility audits, it was discovered that many custom UI components lacked explicit focus indicators when navigated via keyboard (`Tab`), making it difficult or impossible for keyboard-only users to discern their current position on the page. Standard `:focus` can be visually disruptive for mouse/touch users on some elements.
**Action:** Always define explicit `*:focus-visible` styles in the root CSS files for the project's themes (`gold-mold/css/styles.css`, `void-styles.css`, `proposta-styles.css`). Use appropriate contrast colors (e.g., `--accent-clay` for light themes, `white` for dark themes) and ensure `outline-offset` is used to clearly detach the focus ring from the element.
