...
## 2024-07-06 - Respecting Prefers-Reduced-Motion for Smooth Scrolling
**Learning:** The `scrollIntoView({ behavior: 'smooth' })` method was hardcoded across dynamic result boxes (like the "Calcular Oportunidade" functionality), ignoring vestibular accessibility settings. Users with 'prefers-reduced-motion: reduce' were still subjected to smooth scrolling animations.
**Action:** When using `scrollIntoView` in JavaScript, conditionally apply `behavior: 'smooth'` by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` (or using a ternary) to respect vestibular accessibility settings.
