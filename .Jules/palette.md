...
## 2024-06-14 - Missing Keyboard Focus Styles
**Learning:** Custom UI components across the project (buttons, inputs, links) lacked explicit `*:focus-visible` styles in the root CSS files, making keyboard navigation difficult or impossible for users who rely on it without a mouse. Relying on default browser outlines or using `:focus` could sometimes clash with the dark/minimalist design.
**Action:** Always ensure custom UI components have explicit `*:focus-visible` styles defined in the root CSS files (`gold-mold/css/styles.css`, `void-styles.css`, and `proposta-styles.css`), using appropriate contrast colors (e.g., `--accent-clay` for light themes and `white` for dark themes). This ensures keyboard accessibility without disrupting mouse/touch users.
