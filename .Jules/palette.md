...
## 2024-07-10 - Respect Vestibular Accessibility for JS Smooth Scrolling
**Learning:** Hardcoding `{ behavior: 'smooth' }` in `scrollIntoView` overrides user OS-level accessibility preferences, causing potential discomfort for users with vestibular disorders.
**Action:** Always conditionally apply `behavior: 'smooth'` by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches`.
