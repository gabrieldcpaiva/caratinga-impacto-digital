...
## 2024-05-17 - Keyboard Navigation Focus
**Learning:** Custom UI components and minimal themes often lack clear focus indicators, which breaks keyboard navigation accessibility for users who don't use a mouse.
**Action:** Always define explicit `*:focus-visible` styles in root CSS files (`gold-mold/css/styles.css`, `void-styles.css`, `proposta-styles.css`) using high-contrast colors (e.g., `--accent-clay` or `white`) and an `outline-offset` to ensure clear visibility without disrupting mouse click styling.
