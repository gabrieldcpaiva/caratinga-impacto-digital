...
## 2024-07-06 - Respecting Vestibular Accessibility for JS Smooth Scrolling
**Learning:** Hardcoding `behavior: 'smooth'` in `scrollIntoView` calls ignores user operating system preferences for reduced motion, potentially causing discomfort for users with vestibular disorders.
**Action:** Always conditionally apply smooth scrolling in JavaScript by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` before executing animations or transitions.
