...
## 2024-06-20 - Global Focus-Visible Styles
**Learning:** Custom UI components lacked explicit `:focus-visible` styles, hindering keyboard navigation accessibility. Using generic `:focus` can disrupt mouse/touch users.
**Action:** Always ensure explicit `*:focus-visible` styles are defined in root CSS files using appropriate contrast colors (e.g., `--accent-clay` for light themes and `white` for dark themes) to support keyboard navigation without affecting mouse users.
