...
## 2024-07-15 - Vestibular Accessibility & Smooth Scrolling
**Learning:** Hardcoding `behavior: 'smooth'` in `scrollIntoView` animations can trigger nausea or dizziness for users with vestibular disorders. The interface needs to respect the system-level 'prefers-reduced-motion' setting.
**Action:** Always check `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` before applying smooth scrolling or CSS animations to respect user accessibility preferences.
