...
## 2024-07-10 - Respecting Vestibular Accessibility Settings
**Learning:** Hardcoding `behavior: 'smooth'` in `scrollIntoView` calls ignores user preferences and can cause discomfort or nausea for users with vestibular disorders.
**Action:** Always conditionally apply `behavior: 'smooth'` by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` to respect vestibular accessibility settings. If the user prefers reduced motion, fallback to `behavior: 'auto'`.
