...
## 2024-07-16 - Respecting Vestibular Accessibility
**Learning:** Hardcoding `behavior: 'smooth'` in `scrollIntoView` can cause motion sickness for users with vestibular disorders.
**Action:** Always check `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` before applying smooth scrolling in JavaScript.
