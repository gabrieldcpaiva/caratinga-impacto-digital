...
## 2024-10-24 - Respecting Vestibular Accessibility Settings
**Learning:** Hardcoding `behavior: 'smooth'` in JavaScript `scrollIntoView` animations can trigger nausea or discomfort for users with vestibular disorders because it ignores system-level preferences for reduced motion.
**Action:** When using `scrollIntoView` in JavaScript, conditionally apply `behavior: 'smooth'` by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` to respect vestibular accessibility settings.
