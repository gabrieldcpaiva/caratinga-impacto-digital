...
## 2024-05-17 - Keyboard Navigation & Focus Visible
**Learning:** Custom UI components across the different themes (Void Principle and Golden Mold) lack explicit focus indicators for keyboard users. Relying on default browser outlines or `:focus` can disrupt mouse users or blend into the background.
**Action:** Always ensure custom UI components have explicit `*:focus-visible` styles defined in the root CSS files (`void-styles.css`, `proposta-styles.css`, and `gold-mold/css/styles.css`) using appropriate contrast colors (e.g., `white` for dark themes and `--accent-clay` for light themes) and `outline-offset` to guarantee visibility.
