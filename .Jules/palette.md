...
**Learning:** The custom UI components and minimalist design elements across the project lack explicit focus indicators, making it difficult for users relying on keyboard navigation to track their location on the page.
**Action:** Always ensure custom UI components have explicit `*:focus-visible` (not `:focus`) styles defined in the root CSS files to support keyboard navigation accessibility without disrupting mouse/touch users, using appropriate contrast colors (e.g., `--accent-clay` for light themes and `white` for dark themes).
