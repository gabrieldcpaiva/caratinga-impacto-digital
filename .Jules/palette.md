...
## 2024-07-10 - Respecting Vestibular Preferences
**Learning:** Hardcoding `behavior: 'smooth'` in JavaScript methods like `scrollIntoView` forces animations regardless of user system preferences, causing issues for users with vestibular disorders.
**Action:** Always conditionally apply smooth scrolling by checking `!window.matchMedia('(prefers-reduced-motion: reduce)').matches` or `prefers-reduced-motion` in CSS to respect user accessibility settings.
